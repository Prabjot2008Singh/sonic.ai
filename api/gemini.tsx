...
export default async function handler(req: any, res: any) {
    ...
    try {
        const { userInput, languages, apiKey: userApiKey } = req.body;

        // Use the user's key if provided, otherwise fall back to the default key on the server.
        const apiKey = userApiKey || process.env.API_KEY;

        if (!apiKey) {
            return res.status(500).json({ error: 'API key is not configured on the server and none was provided.' });
        }
        ...
        let clientErrorMessage = 'An unknown server error occurred';
        if (error.message && error.message.includes('API key not valid')) {
            clientErrorMessage = 'The provided API key is not valid. Please check it in the settings.';
        } else if (error.message) {
            clientErrorMessage = 'An error occurred while communicating with the AI.';
        }
